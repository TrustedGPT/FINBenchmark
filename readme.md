...
- 金融业务：主要基于实际业务场景的任务设计，评估任务多样，包含以下六个类别
  1. 意图一致性识别：分析两个文本的核心目标、操作路径和信息完整性，判断二者在特定主题下是否表达相同意图。
  2. 用户意图分类：识别金融文本属于何种电销或催收意图。
  3. 业务合规：判断坐席回复的是否违反安全要求。
  4. 动作识别：判断对话内容所属的动作类别。
  5. 文段匹配：评估模型对检索数据进行信息匹配的能力。
  6. 风险评估：评估模型根据用户信息评判潜在金融风险的能力
...
|deepseek-chat-v3|0.6322|0.7364|0.9101|0.7596|2|
...
└── teval_output
   ...
   └── understand.json
```
请将您的文件以压缩包形式提交，例如 gemini-1.5-pro.zip，并发送至 jizh3@mail2.sysu.edu.cn。评估过程完成后，结果将在 GitHub 上公布。
